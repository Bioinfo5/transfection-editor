...
class TabControl {
  constructor(I) { //ID and array of Tabs objects to be passed to the constructor in input
    ...
    this.AfterDelete = (I.AfterDelete || function (i) {
    ...
    if (this.AutoInit) {
      this.init()
    } //Append the buttons on the page directly on construction
  ...
  click(e) { //Action to be taken when click event occurs in a tab header section
    ...
    if (t.hasAttribute("tabaction")) { //Click was on a control
      ...
      if (t.classList.contains("LinkCtrl_Active")) { //The tab is active
        ...
        if (this.Multiple) {
          this.Tabs[index].set("Resting")
        } //In this case, it is fine to hide any items
        else {
          this.Tabs[index].fold()
        } //Fold or unfold the content, one tab should always be active
      } else {
        if (t.classList.contains("LinkCtrl_Disabled")) {
          return
        } //Nothing to do in case the tab is disabled
  ...
  jumpTo(index) { //Jump to the tab with the provided index
    ...
    if (!this.Multiple) { //Only one tab active, need to deactivate previously activated tab
      this.Tabs.forEach(function (t, i) { //Loop the tabs
        if (t.Active && i != index) {
          t.set("Resting")
        } //This tab needs to be deactivated
      });
    }
    return this;
  }
  ...
  addTab(I) { //Create a new tab using the properties passed as an object in input
    ...
    if (this.LastKey == 0) {
      this.init()
    } //If this is the first tab, initialize the tab
    else {
      this.addTabHTML(l)
    } //Add the html for the new tab. This will succeed only if the containers are available
    if (I.SetActive) {
      this.jumpTo(l)
    }
  ...
  addTabHTML(index) { //Internal use. Create the HTML for the tab of the desired index, if the containers exist. The html is simply appended at the end
    ...
    if (!me.classList.contains("LinkCtrl_Tab")) {
      return
    } //The container exist but has not yet been initialized as a tab
  ...
  deleteTab(index) { //Tab deletion
    ...
    if (tabs[index] === undefined) {
      return this
    } //Stop there if the tab at this index does not exist
    if (!this.Multiple && tabs[index].Active) { //In case this was the active tabs and the tabControl is not set on multiple
      if (index == 0 && l > 1) {
        this.jumpTo(1)
      } //Go to the next one if this was the first and something follows
      if (index > 0) {
        this.jumpTo(index - 1)
      } //Otherwise, go to the previous one if possible
    }
    ...
    if (tabs.length == 0) { //If no tabs are left
      ...
      } //Hide the remaining HTML for the ControlTab, if it exists
    }
    return this;
  }

  duplicateConfirm(index) { //Confirmation of tab deletion
    ...
    Form.open({
      ...
      HTML: "<div style=\"text-align: center\">" +
        "<p style=\"color: black;\">This will duplicate the tab and all its contents.</p>" +
      ...
      Buttons: [
        {
          Label: "Ok", Click: function () {
            ...
            if (input.value) {
              ui.AfterDuplicate(index, parseInt(input.value, 10));
            }
            Form.close(id);
          }
        },
